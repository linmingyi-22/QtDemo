...
static inline quint32 toKeyOrUnicode(quint32 vk, quint32 scancode)
{
...
LRESULT CALLBACK LowLevelKeyboardProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    //将VK转换为Qt键代码或unicode字符
    int key = int(toKeyOrUnicode(kbdll->vkCode, kbdll->scanCode));
    //获取当前按下键的字符
    ...
    {
        emit CMyGlobalKeyEvent::getInstance()->keyEvent(new QKeyEvent(QEvent::KeyPress, key, modifiers, kbdll->scanCode, kbdll->vkCode, nativeModifiers, text, autorep));
        break;
    }
    case WM_KEYUP:        // 当释放非系统键
        emit CMyGlobalKeyEvent::getInstance()->keyEvent(new QKeyEvent(QEvent::KeyRelease, key, modifiers, kbdll->scanCode, kbdll->vkCode, nativeModifiers, text, !autorep));
        break;
...
bool CMyGlobalKeyEvent::installKeyEvent()
{
    if(g_hook!=nullptr)
        return true;
...
bool CMyGlobalKeyEvent::removeKeyEvent()
{
    if(!g_hook)
        return true;
