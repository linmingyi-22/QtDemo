...
public:
    ...
    explicit IconHelper(const QString &fontFile, const QString &fontName, QObject *parent = nullptr);
    ...
    //设置图形字体到图标
    static void setPixmap(QAbstractButton *btn, const QColor &color,int icon, quint32 size = 12,quint32 width = 15, quint32 height = 15,int flags = Qt::AlignCenter);

    //获取指定图形字体,可以指定文字大小,图片宽高,文字对齐
    static QPixmap getPixmap(const QColor &color, int icon, quint32 size = 12, quint32 width = 15, quint32 height = 15, int flags = Qt::AlignCenter);
...
private:
    QFont m_fontIcon;                //图形字体
    QList<QAbstractButton *> m_ListBtns;   //按钮队列
    QList<QPixmap> m_ListPixNormal;       //正常图片队列
    QList<QPixmap> m_ListPixHover;        //悬停图片队列
    QList<QPixmap> m_ListPixPressed;      //按下图片队列
    QList<QPixmap> m_ListPixChecked;      //选中图片队列
...
public:
    ...
    //设置图形字体到图标
    void setPixmap1(QAbstractButton *btn, const QColor &color,int icon, quint32 size = 12,quint32 width = 15, quint32 height = 15,int flags = Qt::AlignCenter);

    //获取指定图形字体,可以指定文字大小,图片宽高,文字对齐
    QPixmap getPixmap1(const QColor &color, int icon, quint32 size = 12,quint32 width = 15, quint32 height = 15,int flags = Qt::AlignCenter);
