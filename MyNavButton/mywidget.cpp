...
{
    m_listIcons<<0xf17b<<0xf002<<0xf013<<0xf021<<0xf0e0;//横排的五个图标

    //默认显示的按钮
    ui->navButton13->setChecked(true);
...
void CMyWidget::initBtn1()
{
    quint32 size = 15;//图标大小
    quint32 pixWidth = 25;
    quint32 pixHeight = 25;
    ...
    for (int i = 0; i < btns1.count(); i++) {
        CMyNavButton *btn = btns1.at(i);
        btn->setPaddingLeft(32);//为图标预留空间
        btn->setLineSpace(6);//按钮
...
void CMyWidget::initBtn3()
{
    ...
    for (int i = 0; i < btns3.count(); i++) {
        CMyNavButton *btn = btns3.at(i);
        btn->setPaddingLeft(35);
        btn->setLineWidth(10);
        btn->setLineColor(QColor("#029FEA"));
        btn->setShowTriangle(true);
        btn->setTextAlign(CMyNavButton::TextAlign_Left);
        btn->setTrianglePosition(CMyNavButton::TrianglePosition_Left);
        btn->setLinePosition(CMyNavButton::LinePosition_Right);
...
void CMyWidget::initBtn4()
{
    ...
    for (int i = 0; i < btns4.count(); i++) {
        CMyNavButton *btn = btns4.at(i);
        btn->setLineSpace(10);
        btn->setLineWidth(10);
        btn->setPaddingRight(35);
        btn->setShowTriangle(true);
        btn->setTextAlign(CMyNavButton::TextAlign_Right);
        btn->setTrianglePosition(CMyNavButton::TrianglePosition_Left);
        btn->setLinePosition(CMyNavButton::LinePosition_Right);
...
void CMyWidget::initBtn5()
{
    ...
    for (int i = 0; i < btns5.count(); i++) {
        CMyNavButton *btn = btns5.at(i);
        btn->setFont(font);
        btn->setPaddingLeft(20);
        btn->setShowLine(false);
        btn->setTextAlign(CMyNavButton::TextAlign_Center);
        btn->setLinePosition(CMyNavButton::LinePosition_Bottom);
...
void CMyWidget::initBtn7()
{
    ...
    for (int i = 0; i < btns7.count(); i++) {
        CMyNavButton *btn = btns7.at(i);
        btn->setFont(font);
        btn->setPaddingLeft(0);
        btn->setLineSpace(0);
        btn->setShowTriangle(true);
        btn->setTextAlign(CMyNavButton::TextAlign_Center);
        btn->setTrianglePosition(CMyNavButton::TrianglePosition_Bottom);
        btn->setLinePosition(CMyNavButton::LinePosition_Top);
...
void CMyWidget::buttonClick1()
{
    CMyNavButton *b=(CMyNavButton *)sender();
